...
from .serializers import EmployeeTaskSerializer, HelperTaskSerializer, HelperTaskCreateSerializer
...
class EmployeeTaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = EmployeeTask.objects.select_related(
            'employee', 'stage__workshop', 'stage__order', 'stage__order_item__product'
        ...
        if workshop:
            queryset = queryset.filter(stage__workshop_id=workshop)
        ...
        # Фильтрация по order_item
        order_item = self.request.query_params.get('order_item', None)
        if order_item:
            queryset = queryset.filter(stage__order_item_id=order_item)
    ...
    def complete(self, request, pk=None):
        ...
        completed_quantity = request.data.get('completed_quantity', task.quantity)
        defective_quantity = request.data.get('defective_quantity', 0)
...
class HelperTaskViewSet(viewsets.ModelViewSet):
    queryset = HelperTask.objects.all()
    serializer_class = HelperTaskSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        queryset = HelperTask.objects.select_related(
            'helper', 'employee_task__employee', 'employee_task__stage__workshop', 
        ...
        # Фильтрация по помощнику
        helper = self.request.query_params.get('helper', None)
        if helper:
            queryset = queryset.filter(helper_id=helper)
        
        # Фильтрация по цеху помощника
        workshop = self.request.query_params.get('workshop', None)
        if workshop:
            queryset = queryset.filter(employee_task__stage__workshop_id=workshop)
        
        # Фильтрация по сотруднику, которому помогают
        employee = self.request.query_params.get('employee', None)
        if employee:
            queryset = queryset.filter(employee_task__employee_id=employee)
        
        return queryset

    def get_serializer_class(self):
        if self.action == 'create':
            return HelperTaskCreateSerializer
        return HelperTaskSerializer
    ...
    def available_tasks(self, request):
        """Получает доступные задачи для помощника в его цехе"""
        helper_id = request.query_params.get('helper', None)
        if not helper_id:
            return Response({'error': 'helper parameter required'}, status=400)
        
        try:
            helper = User.objects.get(id=helper_id, role=User.Role.HELPER)
        except User.DoesNotExist:
            return Response({'error': 'Helper not found'}, status=404)
        
        if not helper.workshop:
            return Response({'error': 'Helper has no assigned workshop'}, status=400)
        
        # Получаем все задачи сотрудников в цехе помощника
        employee_tasks = EmployeeTask.objects.filter(
            stage__workshop=helper.workshop,
            employee__role=User.Role.WORKER
        ).select_related(
            'employee', 'stage__workshop', 'stage__order', 'stage__order_item__product'
        ).prefetch_related('stage__order__client')
        
        # Исключаем задачи, которые уже имеют помощника
        existing_helper_tasks = HelperTask.objects.filter(
            employee_task__in=employee_tasks,
            helper=helper
        ).values_list('employee_task_id', flat=True)
        
        available_tasks = employee_tasks.exclude(id__in=existing_helper_tasks)
        
        # Сериализуем доступные задачи
        from .serializers import EmployeeTaskSerializer
        serializer = EmployeeTaskSerializer(available_tasks, many=True)
